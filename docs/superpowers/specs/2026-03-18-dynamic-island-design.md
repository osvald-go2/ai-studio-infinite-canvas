...
- **胶囊态检测**：Main Process 使用 `setInterval` 每 100ms 调用 `screen.getCursorScreenPoint()` 获取鼠标位置。当鼠标进入刘海触发区域（屏幕顶部居中，约 200x40px 范围）时，切换到通知卡片态。此时 Notch Window 设置 `setIgnoreMouseEvents(true)`，对用户完全透明。
